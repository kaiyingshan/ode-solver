...
__all__ = [
   "Procedure", "find_root", "sec_order_euler", "solve_ivp", "red_order", "Wronskian", "variation_of_parameters", "to_std", "to_general", "first_order_separable", "first_order_linear", "first_order_homogeneous", "first_order_autonomous", "first_order_exact", "first_order_bernoulli", "undetermined_coefficients", "nth_order_const_coeff", "t", "system", "nonhomo_system_variation_of_parameters", "phase_portrait"
]
...
def phase_portrait(coeffs: List[List[int]], xmin=-5, xmax=5, ymin=5, ymax=-5, steps=10, plot_type="stream"):
    import numpy as np
    ...
    for i in range(numx):
        for j in range(numy):
            gradient = np.matmul(matrix, [[X[i, j]], [Y[i, j]]])
            u[i, j] = gradient[0]
            v[i, j] = gradient[1]
